<template>
  <div class="card-page">
    ...
    <div
      class="row d-flex justify-content-center mt-5 mb-5"
      ...
      <div class="list-group col-9">
        ...
        >
          ...
          <div class="row">
            ...
            <div class="mr-4">
              ...
              <p class="font-text">
                R$
                {{ (item.preco * item.quantidade).toLocaleString(2) }}
              </p>
            </div>
            <div>
              <p>Quantidade</p>
        ...
        <p
          ...
          v-if="
            retirada &&
            this.carrinho[0] &&
            this.carrinho[0].fornecedor.enderecos
          "
        >
          <b>Endereço da loja: </b>
          {{ this.carrinho[0].fornecedor.enderecos[0].rua }},
          {{ this.carrinho[0].fornecedor.enderecos[0].numero }}
        </p>
        <br />
        ...
        <select
          ...
          @change="onChangePagamento($event)"
        >
        ...
        <br />
        <label class="h5" v-if="entrega || retirada" for="formaEntrega"
          >Forma de entrega:</label
        >
        <select
          v-if="entrega || retirada"
          name="formaEntrega"
          id="formaEntrega"
        ...
        <select
          v-model="enderecoEscolhido"
          name="enderecoEscolhido"
          id="enderecoEscolhido"
          v-if="
            entrega &&
            this.comprador.enderecos &&
            this.formaEntrega === 'ENTREGA'
        ...
        <br />
        <label class="h5" v-if="manha || tarde || noite" for="horarioEntrega"
          >Horário de entrega:</label
        >
        <select
          v-if="manha || tarde || noite"
          name="horarioEntrega"
          id="horarioEntrega"
          @change="onChangeHorario($event)"
        >
          <option
            v-if="
              (manha && this.dataHoje.demanha) ||
              (manha &&
                !this.dataHoje.demanha &&
                !this.dataHoje.tarde &&
                !this.dataHoje.noite)
            "
            value="MANHA"
        ...
        <p
          ...
          class="h5"
        >
          <b>A entrega/retirada deve ser efetuada no dia seguinte à compra. </b>
        </p>
        <button
          @click="checkout()"
          type="button"
  ...
  methods: {
    ...
    rmProduto(id) {
      ...
      document.getElementById("precoTotal").innerHTML =
        "R$" + this.precoTotal.toLocaleString(2);
    },
  },
  mounted() {
    ...
    this.carrinho[0].fornecedor.horarios.forEach((horario) => {
      if (horario.horarioSelecionado === "MANHA")
        this.manha = horario.horarioSelecionado;
      if (horario.horarioSelecionado === "TARDE")
        this.tarde = horario.horarioSelecionado;
      if (horario.horarioSelecionado === "NOITE")
        this.noite = horario.horarioSelecionado;
    });

    if (
      this.carrinho[0].fornecedor.infoEntrega === "ENTREGA" ||
      this.carrinho[0].fornecedor.infoEntrega === "ENTREGA_E_RETIRADA"
    )
      this.entrega = this.carrinho[0].fornecedor.infoEntrega;
    if (
      this.carrinho[0].fornecedor.infoEntrega === "RETIRADA" ||
      this.carrinho[0].fornecedor.infoEntrega === "ENTREGA_E_RETIRADA"
    )
      this.retirada = this.carrinho[0].fornecedor.infoEntrega;
