<template>
  <div class="card mb-4 shadow-sm">
    ...
    <div class="card-body">
      <p class="font-text" style="font-size: 22px !important">
        <b>Pedido #{{ pedido.id }}</b>
      </p>
      <p class="font-text" style="font-size: 18px !important">
        Valor: R$ {{ pedido.valor.toLocaleString(2) }}
      </p>
      <p class="font-text" style="font-size: 18px !important">
        Data de entrega: {{ pedido.dataEntrega }}
      </p>
      <p class="font-text" style="font-size: 18px !important">
        Status: {{ pedido.status.replace(/_/g, " ") }}
      </p>
      <p class="font-text" style="font-size: 18px !important">
        Método de pagamento: {{ pedido.metodoPagamento.replace(/_/g, " DE ") }}
      </p>
      <router-link
        v-if="Object.keys(comprador).length !== 0"
        type="button"
        class="btn btn-primary product-button details"
